...
export const getAlumnoById = async (id) => {
  try {
    const response = await api.get(`/webhook/get-alumno-by-id?id=${id}`);
...
export const registrarPago = async (datospago) => {
  try {
    const response = await api.post(`/webhook/register-pago`, datospago);
...
export const getAlumnosList = async (params = {}) => {
  try {
    ...
    const response = await api.get(`/webhook/get-alumnos-list?${queryString}`);
    return response.data;
  } catch (error) {
...
export const updateCupoHorario = async (horarioId, accion) => {
  try {
    const response = await api.post(`/webhook/update-cupo-horario`, {
