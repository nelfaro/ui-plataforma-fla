-- ============================================
-- Setup: Nuevo sistema de estados (FRIO/TIBIO/CALIENTE/ACTIVO)
-- ============================================

-- 1. Limpiar datos viejos (opcional - descomentar si es necesario)
-- TRUNCATE TABLE pagos CASCADE;
-- TRUNCATE TABLE conversation_logs CASCADE;
-- TRUNCATE TABLE alumnos CASCADE;

-- 2. Alterar el constraint de estado
ALTER TABLE alumnos DROP CONSTRAINT alumnos_estado_check;
ALTER TABLE alumnos ADD CONSTRAINT alumnos_estado_check
  CHECK (estado = ANY (ARRAY['FRIO'::text, 'TIBIO'::text, 'CALIENTE'::text, 'ACTIVO'::text]));

-- 3. Insertar datos de prueba para testing con el nuevo sistema de estados
-- Distribución: 30 registros totales
-- - RECOMENDACION: 8 registros (2 FRIO, 3 TIBIO, 3 CALIENTE, 2 ACTIVO)
-- - INSTAGRAM: 6 registros (2 FRIO, 2 TIBIO, 1 CALIENTE, 1 ACTIVO)
-- - FACEBOOK: 4 registros (1 FRIO, 1 TIBIO, 1 CALIENTE, 1 ACTIVO)
-- - TIKTOK: 3 registros (1 FRIO, 1 TIBIO, 0 CALIENTE, 1 ACTIVO)
-- - ORGANICO: 3 registros (1 FRIO, 0 TIBIO, 1 CALIENTE, 1 ACTIVO)
...
SELECT COUNT(*) as total_records,
       COUNT(DISTINCT origen) as unique_origins,
       origen
FROM alumnos
